...
export default function DemoDash() {
  ...
  useEffect(() => {
    if (user) {
      loadDashboardData();
      loadTeamMembersForHeader();
    }
  }, [user]);

  const loadTeamMembersForHeader = async () => {
    const token = localStorage.getItem('token');
    if (!token) return;
  ...
  const loadDashboardData = async () => {
    ...
    if (cached && cacheTimestamp && (Date.now() - parseInt(cacheTimestamp)) < cacheTime) {
      ...
      setDashboardProjects(cachedData.dashboardProjects);
      setDashboardStats(cachedData.dashboardStats);
      setProjectTasks(cachedData.projectTasks);
      setPieChartData(cachedData.pieChartData);
      setTeamPerformanceData(cachedData.teamPerformanceData);
      setPendingTasksForDisplay(cachedData.pendingTasksForDisplay);
      setDashboardActivities(cachedData.dashboardActivities || []);
      setBottleneckData(cachedData.bottleneckData || { overloadedMembers: [], delayedTasks: [], aiRecommendations: [] });
    ...
    try {
      ...
      const userProjects = projectsRes.ok ? (await projectsRes.json()).projects || [] : [];
      const members = membersRes.ok ? (await membersRes.json()).members || [] : [];

      setDashboardProjects(userProjects.slice(0, 10));
      setDashboardStats(prev => ({ ...prev, activeProjects: userProjects.length, members: members.length }));

      // Load all tasks in parallel
      const allTasks = [];
      const tasksMap = {};
      const taskPromises = userProjects.map(async (project) => {
      ...
      tasksResults.forEach(tasks => allTasks.push(...tasks));
      ...
      const pendingTasksList = allTasks.filter(t => t.status === 'pending' || t.status === 'pending_approval');
      ...
      members.forEach(m => {
        memberWorkload[m.name || m.email] = {
          name: m.name || m.email,
          capacity: m.capacity || 40,
          currentLoad: m.current_load || 0,
          idlePercentage: m.idle_percentage || 0,
          tasks: []
        };
      });

      allTasks.forEach(t => {
        if (t.assigned_to && t.assigned_to !== 'Unassigned' && t.status !== 'completed') {
          if (memberWorkload[t.assigned_to]) {
            memberWorkload[t.assigned_to].tasks.push(t);
          }
        }
      });

      const overloadedMembers = Object.values(memberWorkload).filter(m => m.idlePercentage < 20);
      const idleMembers = Object.values(memberWorkload).filter(m => m.idlePercentage > 60);
      
      const bottleneckCount = delayedTasks.length + overloadedMembers.length;
      ...
      if (overloadedMembers.length > 0 && idleMembers.length > 0) {
        recommendations.push({
          type: 'reassign',
          message: `Reassign tasks from ${overloadedMembers[0].name} to ${idleMembers[0].name} to balance workload`
        });
      }
      if (delayedTasks.length > 0) {
        recommendations.push({
          type: 'urgent',
          message: `${delayedTasks.length} task${delayedTasks.length > 1 ? 's are' : ' is'} overdue - prioritize immediately`
        });
      }
      if (overloadedMembers.length > 0) {
        recommendations.push({
          type: 'extend',
          message: `Consider extending deadlines for ${overloadedMembers[0].name}'s tasks`
        });
      }
      ...
      setBottleneckData({
        overloadedMembers,
        idleMembers,
        delayedTasks: delayedTasks.slice(0, 5),
      ...
      // Sort all tasks by updated_at to get most recent activities
      ...
      // Process tasks to create activity entries
      sortedTasks.slice(0, 20).forEach(task => {
        ...
        if (task.status === 'completed' || task.status === 'done') {
          recentActivities.push({
            type: 'completed',
            user: task.assigned_to || 'Team member',
        ...
        // Activity for follow-ups sent
        if (task.last_followup_at) {
          recentActivities.push({
            type: 'followup',
            user: task.assigned_to || 'Team member',
            task: task.title || task.task || 'Untitled task',
            project: task.project_name || 'Unknown project',
            time: timeAgo(task.last_followup_at)
          });
        }
        
        // Activity for task assignments (when task was created/assigned)
        if (task.assigned_to && task.assigned_to !== 'Unassigned' && task.status !== 'completed') {
          recentActivities.push({
            type: 'assigned',
      ...
      // Sort activities by most recent and limit to 15
      ...
      setDashboardActivities(sortedActivities);
      ...
      sessionStorage.setItem(cacheKey, JSON.stringify({
        dashboardProjects: userProjects.slice(0, 10),
        dashboardStats: stats,
        projectTasks: tasksMap,
        pieChartData: pieData,
        teamPerformanceData: performanceData,
        pendingTasksForDisplay: pendingDisplay,
        dashboardActivities: sortedActivities,
        bottleneckData: {
          overloadedMembers,
          idleMembers,
          delayedTasks: delayedTasks.slice(0, 5),
  ...
  const loadProjects = async () => {
    ...
    const cacheTime = sessionStorage.getItem('projects_cache_time');
    ...
    try {
      ...
      if (response.ok) {
        ...
        sessionStorage.setItem('projects_cache', JSON.stringify(projectsList));
  ...
  const getCartoonAvatar = (name) => {
    const colors = [
      ...
      { bg: 'from-blue-400 to-cyan-400', emoji: '🐱' },
      { bg: 'from-green-400 to-emerald-400', emoji: '🐸' },
      { bg: 'from-yellow-400 to-orange-400', emoji: '🦊' },
      { bg: 'from-red-400 to-pink-400', emoji: '🐼' },
      { bg: 'from-indigo-400 to-purple-400', emoji: '🐨' },
      { bg: 'from-teal-400 to-cyan-400', emoji: '🐧' },
      { bg: 'from-orange-400 to-red-400', emoji: '🦁' },
    ];
  ...
  return (
    <>
          ...
          {!sidebarCollapsed && (() => {
            ...
            return (
              <div className="pt-3 bg-[#111111]">
                <p className="text-xs text-gray-500 px-3 pb-2">Recent Projects</p>
                {recentProjects.length > 0 ? (
                  ...
                  <p className="text-xs text-gray-600 px-3 py-2">No recent projects</p>
                )}
              </div>
            );
          })()}
          ...
          {!sidebarCollapsed ? (
            <>
              <button 
                ...
                className={`w-full flex items-center gap-3 px-3 py-2 text-sm ${activePage === 'settings' ? 'text-white bg-[#1a1a1a]' : 'text-gray-400 hover:text-white'} hover:bg-[#1a1a1a] rounded-lg transition-colors`}
              >
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              ...
              <div className="flex items-center gap-3 px-3 py-3 mt-2 hover:bg-[#1a1a1a] rounded-lg transition-colors cursor-pointer">
                {(() => {
                  ...
                  const avatar = getCartoonAvatar(displayName);
                  return (
                    <div className={`w-8 h-8 bg-gradient-to-br ${avatar.bg} rounded-full flex items-center justify-center text-lg flex-shrink-0`}>
                      {avatar.emoji}
                    </div>
                  );
                })()}
            ...
            <div className="flex flex-col items-center gap-3">
              <button 
                onClick={() => handleMenuItemClick('settings')}
                className={`p-2 ${activePage === 'settings' ? 'text-white bg-[#1a1a1a]' : 'text-gray-400 hover:text-white'} hover:bg-[#1a1a1a] rounded-lg transition-colors`}
              >
              ...
              {(() => {
                ...
                const avatar = getCartoonAvatar(displayName);
                return (
                  <div className={`w-10 h-10 bg-gradient-to-br ${avatar.bg} rounded-full flex items-center justify-center text-xl cursor-pointer hover:scale-110 transition-transform`}>
                    {avatar.emoji}
                  </div>
                );
              })()}
      ...
      {/* Main Content */}
      <div 
        className={`main-scroll flex-1 relative z-10 smooth-scroll ${activePage === 'projects' ? 'overflow-hidden' : 'overflow-auto'} transition-all duration-300`}
        style={{ 
          marginLeft: isProjectsPanelOpen ? '16rem' : '0'
        }}
      >
            ...
            <div className="flex -space-x-2 py-2.5">
              {teamMembers.slice(0, 3).map((member, idx) => {
                ...
                return (
                  <div 
                    key={idx}
                    className={`w-8 h-8 rounded-full bg-gradient-to-br ${avatar.bg} border-2 border-[#0a0a0a] ring-1 ring-white/10 shadow-lg flex items-center justify-center text-lg`}
              ...
              {teamMembers.length === 0 && (
                <>
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-400 to-pink-400 border-2 border-[#0a0a0a] ring-1 ring-white/10 shadow-lg flex items-center justify-center text-lg">🦄</div>
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-400 to-cyan-400 border-2 border-[#0a0a0a] ring-1 ring-white/10 shadow-lg flex items-center justify-center text-lg">🐱</div>
                </>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6 mb-8 items-stretch">
            ...
            <div className="bg-[#0f0f0f]/60 backdrop-blur-xl border border-[#1f1f1f]/50 rounded-xl p-6 hover:bg-[#111111]/70 hover:border-[#2a2a2a] transition-all duration-300 flex flex-col">
              <div className="flex items-center justify-between mb-6 flex-shrink-0">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs px-2 py-1 bg-yellow-500/20 text-yellow-400 rounded-lg">AI Powered</span>
                </div>
              </div>
              <div className="space-y-4 overflow-y-auto custom-scrollbar flex-1" style={{ maxHeight: '500px' }}>
                {/* Overloaded Members */}
                {bottleneckData.overloadedMembers.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-xs font-semibold text-red-400 uppercase tracking-wider">⚠️ Overloaded Team Members</p>
                    {bottleneckData.overloadedMembers.map((member, i) => (
                      <div key={i} className="p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm font-semibold text-white">{member.name}</span>
                          <span className="text-xs text-red-400">{member.idlePercentage.toFixed(0)}% idle</span>
                        </div>
                        <p className="text-xs text-gray-400">{member.tasks.length} active tasks • {member.currentLoad}h / {member.capacity}h</p>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {/* Delayed Tasks */}
                {bottleneckData.delayedTasks.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-xs font-semibold text-orange-400 uppercase tracking-wider">🔥 Overdue Tasks</p>
                    {bottleneckData.delayedTasks.map((task, i) => (
                      <div key={i} className="p-3 bg-orange-500/10 border border-orange-500/20 rounded-lg">
                        <p className="text-sm font-semibold text-white mb-1">{task.title || task.task}</p>
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-gray-400">{task.assigned_to || 'Unassigned'}</span>
                          <span className="text-orange-400">Due: {new Date(task.deadline).toLocaleDateString()}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {/* Idle Members */}
                {bottleneckData.idleMembers && bottleneckData.idleMembers.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-xs font-semibold text-green-400 uppercase tracking-wider">✨ Available Team Members</p>
                    {bottleneckData.idleMembers.slice(0, 3).map((member, i) => (
                      <div key={i} className="p-3 bg-green-500/10 border border-green-500/20 rounded-lg">
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-semibold text-white">{member.name}</span>
                          <span className="text-xs text-green-400">{member.idlePercentage.toFixed(0)}% available</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {/* AI Recommendations */}
                {bottleneckData.aiRecommendations.length > 0 && (
                  <div className="space-y-2 mt-4 pt-4 border-t border-[#1f1f1f]">
                    <p className="text-xs font-semibold text-blue-400 uppercase tracking-wider">💡 AI Recommendations</p>
                    {bottleneckData.aiRecommendations.map((rec, i) => (
                ...
                {/* No Bottlenecks */}
                {bottleneckData.overloadedMembers.length === 0 && bottleneckData.delayedTasks.length === 0 && (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm leading-relaxed">
                          {activity.type === 'followup' ? (
                            <>
                              <span className="text-blue-400 font-semibold">Follow-up sent</span>
                              <span className="text-gray-400"> to </span>
                              <span className="font-semibold text-white group-hover:text-[#6B5CE6] transition-colors duration-200">{activity.user}</span>
                              <span className="text-gray-400"> for task: </span>
                              <span className="text-gray-300">"{activity.task}"</span>
                            </>
                          ) : activity.type === 'completed' ? (
                            <>
                              <span className="font-semibold text-white group-hover:text-[#6B5CE6] transition-colors duration-200">{activity.user}</span>
                              <span className="text-gray-400"> completed task: </span>
                              <span className="text-green-400 font-semibold">"{activity.task}"</span>
                            </>
                          ) : activity.type === 'assigned' ? (
                            <>
                              <span className="text-purple-400 font-semibold">Task assigned</span>
                              <span className="text-gray-400"> to </span>
                              <span className="font-semibold text-white group-hover:text-[#6B5CE6] transition-colors duration-200">{activity.user}</span>
                        ...
                        {activity.project && (
                          <p className="text-xs text-gray-500 mt-1">Project: {activity.project}</p>
                        )}
                      </div>
                    </div>
                  );
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-4">
                    {(() => {
                      ...
                      const avatar = getCartoonAvatar(displayName);
                      return (
                        <div className={`w-20 h-20 bg-gradient-to-br ${avatar.bg} rounded-full flex items-center justify-center text-4xl`}>
                          {avatar.emoji}
                        </div>
                      );
                    })()}
                    <div>
                      ...
                      <p className="text-lg font-semibold">{userName || user?.name || user?.email?.split('@')[0] || 'User'}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      readOnly
                      className="w-full px-4 py-3 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg text-white outline-none"
                    />
                  </div>
                  ...
                  {user?.id && (
                    <div>
                      ...
                      <input
                        ...
                        value={user.id}
                        readOnly
                        className="w-full px-4 py-3 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg text-white outline-none font-mono text-sm"
                      />
                    </div>
                  )}
                  ...
                  {githubConnected && (
                    <div>
                      <label className="block text-sm text-gray-400 mb-2">GitHub Connection</label>
                      <div className="px-4 py-3 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg">
                        <span className="inline-flex items-center gap-2 text-white">
                          <span className="w-2 h-2 rounded-full bg-green-400"></span>
                          Connected
                  ...
                  {slackConnected && (
                    <div>
                      <label className="block text-sm text-gray-400 mb-2">Slack Connection</label>
                      <div className="px-4 py-3 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg">
                        <span className="inline-flex items-center gap-2 text-white">
                          <span className="w-2 h-2 rounded-full bg-green-400"></span>
                          Connected
              ...
              <div className="bg-[#0f0f0f]/60 backdrop-blur-xl border border-[#1f1f1f]/50 rounded-xl p-6 hover:bg-[#111111]/70 hover:border-[#2a2a2a] transition-all">
                ...
                <div className="space-y-3">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 rounded-lg text-red-400 hover:text-red-300 transition-all font-medium"
                  >
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                      <path d="M11 4L14 7M14 7L11 10M14 7H6M9 13H4C3.5 13 3 12.5 3 12V4C3 3.5 3.5 3 4 3H9" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                    </svg>
