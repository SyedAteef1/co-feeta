...
⏱️ *Estimated Time:* {estimated_hours} {f"({timeline})" if timeline else ""}"""
            ...
            try:
                ...
                if slack_response.get("ok"):
                    approved_count += 1
                    # Store Slack metadata for follow-ups
                    update_task(task_id, {
...
@project_bp.route("/tasks/<task_id>/follow-up", methods=["POST"])
def send_task_followup(task_id):
    """Send follow-up message to check on task progress"""
    auth_header = request.headers.get('Authorization')
    if not auth_header:
        return jsonify({"error": "No authorization provided"}), 401
    ...
    try:
        ...
        # Check if task has Slack metadata
        ...
        if not slack_channel_id or not slack_user_id:
            return jsonify({"error": "Task not sent to Slack yet"}), 400
...
Please share a quick status update when you can. Thanks! 👍"""
        ...
        if response.json().get("ok"):
            # Update task with last follow-up time
            update_task(task_id, {"last_followup_at": datetime.utcnow()})
            return jsonify({"ok": True, "message": "Follow-up sent"})
        else:
            return jsonify({"error": "Failed to send follow-up"}), 500
